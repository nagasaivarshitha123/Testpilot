...
import { TestCaseFiltersComponent } from "src/app/dialogboxes/test-case-filters/test-case-filters.component";
...
export class ReportsComponent implements OnInit, OnDestroy {
  ...
  filtersCaseSubject: Subscription;
  ...
  constructor(
    ...
    if (!this.filterSubject) {
      this.filterSubject = this.SharedService.filterSubject.subscribe(
        (data: any) => {
          const { items } = data;
    ...
    this.SharedService.searchTextEmitterSubject.subscribe((data: any) => {
  ...
  ngOnInit() {
    ...
    if (!this.filtersCaseSubject) {
      this.filtersCaseSubject =
        this.SharedService.filtersCaseSubject$.subscribe((data: any) => {
          if (data) {
  ...
  onChangePage(e: PageEvent) {
    ...
    this.caseService
      .filterTestCaseReportsSearch(this.filterTestCase)
      .subscribe((data: any) => {
        const { items } = data;
        this.ELEMENT_DATA = items;
        this.length = data.totalElements;
  ...
  filterDialog() {
    const dialogRef = this.dialog.open(TestCaseFiltersComponent, {
      width: "400px",
      height: "380px",
    });
  ...
  searchTextFilter() {
    this.caseService
      ...
      .subscribe((data: any) => {
        this.length = data.totalElements;
        this.currentPageIndex = 0;
        this.SharedService.filterSubject.next(data);
      });
  }

  ngOnDestroy(): void {
    this.filterSubject.unsubscribe();
    this.filtersCaseSubject.unsubscribe();
  }
}
